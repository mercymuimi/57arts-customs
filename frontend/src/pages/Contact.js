...
const C = {
  ...
  error: 'rgba(224,92,92,0.4)', errorText: '#e05c5c',
};

const subjects = [
  'Custom Art Commission', 'Order Inquiry', 'Shipping & Delivery',
  'Payment Issue', 'Vendor Partnership', 'Press & Media', 'General Question',
];
...
const socials = [
  { platform: 'Instagram', handle: '@57artscustoms', href: 'https://instagram.com/57artscustoms' },
  { platform: 'Twitter / X', handle: '@57arts',        href: 'https://x.com/57arts' },
  { platform: 'TikTok',      handle: '@57artscustoms', href: 'https://tiktok.com/@57artscustoms' },
];

// ── WhatsApp number (international format, no + or spaces) ─────────────────
const WHATSAPP_NUMBER = '254700000000'; // ← replace with your real number
const WHATSAPP_LINK   = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Hi 57 Arts! I have a question about')}`;
...
const Contact = () => {
  ...
  const errStyle = { color: C.errorText, fontSize: 11, marginTop: 4 };
    ...
    <div style={{ backgroundColor: C.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', maxWidth: 400, padding: '0 24px' }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, lineHeight: 1.8, marginBottom: 28 }}>
          Thanks <strong style={{ color: C.cream }}>{form.name}</strong>! We'll reply to{' '}
          <strong style={{ color: C.cream }}>{form.email}</strong> within 24 hours.{' '}
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 48px', display: 'grid', gridTemplateColumns: '1fr 420px', gap: 60 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                ...
                <input
                  ...
                  onBlur={e => e.target.style.borderColor = errors.name ? C.error : C.border}
                />
            ...
            <div>
              ...
              <select
                ...
                onFocus={e => e.target.style.borderColor = C.bHov}
                onBlur={e => e.target.style.borderColor = errors.subject ? C.error : C.border}
              >
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Tell us what you need..." rows={6}
                style={{ ...inp('message'), resize: 'vertical', lineHeight: 1.7 }}
                onFocus={e => e.target.style.borderColor = C.bHov}
                onBlur={e => e.target.style.borderColor = errors.message ? C.error : C.border}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                {errors.message
                  ? <p style={errStyle}>{errors.message}</p>
                  : <span />}
                <span style={{ color: form.message.length < 20 ? C.muted : C.gold, fontSize: 11 }}>
            ...
            {/* API-level error */}
            {apiError && (
              <div style={{ backgroundColor: 'rgba(224,92,92,0.08)', border: `1px solid ${C.error}`, borderRadius: 10, padding: '12px 15px' }}>
                <p style={{ color: C.errorText, fontSize: 13 }}>⚠ {apiError}</p>
              </div>
            )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24 }}>
            <span style={{ color: C.gold, fontSize: 16, display: 'block', marginBottom: 12 }}>◈</span>
            <p style={{ color: C.muted, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Email Us</p>
            <a href="mailto:hello@57arts.com" style={{ color: C.cream, fontWeight: 900, fontSize: 15, display: 'block', marginBottom: 3, textDecoration: 'none' }}>
              hello@57arts.com
            </a>
            <p style={{ color: C.muted, fontSize: 12 }}>We reply within a few hours</p>
          </div>

          {/* WhatsApp — fully functional click-to-chat */}
          <a
            href={WHATSAPP_LINK}
            target="_blank"
            rel="noopener noreferrer"
            style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24, textDecoration: 'none', display: 'block', transition: 'border-color 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.borderColor = '#25D366'}
            onMouseLeave={e => e.currentTarget.style.borderColor = C.border}
          >
            <span style={{ color: C.gold, fontSize: 16, display: 'block', marginBottom: 12 }}>◇</span>
            <p style={{ color: C.muted, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>WhatsApp</p>
            <p style={{ color: C.cream, fontWeight: 900, fontSize: 15, marginBottom: 3 }}>+254 700 000 000</p>
            <p style={{ color: '#25D366', fontSize: 12 }}>Tap to chat → Mon–Fri, 9am–6pm EAT</p>
          </a>

          {/* Location */}
          <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24 }}>
            <span style={{ color: C.gold, fontSize: 16, display: 'block', marginBottom: 12 }}>◉</span>
            <p style={{ color: C.muted, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Location</p>
            <p style={{ color: C.cream, fontWeight: 900, fontSize: 15, marginBottom: 3 }}>Nairobi, Kenya</p>
            <p style={{ color: C.muted, fontSize: 12 }}>Westlands, Nairobi CBD</p>
          </div>
          ...
          <div style={{ backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24 }}>
            ...
            {socials.map(({ platform, handle, href }) => (
              <a
                key={platform} href={href} target="_blank" rel="noopener noreferrer"
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${C.border}`, textDecoration: 'none' }}
              >
